...
#include <sstream>
using namespace std;
//https://github.com/Pixy07/allamvizsga.git
...
__global__ void gpuMM(float *A, float *C, int N)
{
    int row = blockIdx.x;
    int col = threadIdx.x;
...
__global__ void normalize(int N,float *hA)
{
...
__global__ void symmetrize(int N, float *hA)
{
    float newVal;
    int row = blockIdx.x;
    int col = threadIdx.x;
...
__global__ void inflate(int N, float *hA)
{
    float infRate=1.5;
    int row = blockIdx.x;
    int col = threadIdx.x;
...
*/
   ...
   HANDLE_ERROR( cudaMemcpy(dA,hA,size,cudaMemcpyHostToDevice));
    int n=3;
    cout << "1 without dim ; 2 with dim ;0 to stop"<<endl;
